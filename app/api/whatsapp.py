                    ...
                    try:
                        ...
                        if not person_bytes:
                            await send_whatsapp_reply_cloud(
                                    to_waid=from_waid,
                                    body=f"Please send a clear front photo of the person first.",
                                    phone_number_id=outbound_pnid,
                                )
                        ...
                        if not garment_bytes:
                            await send_whatsapp_reply_cloud(
                                    to_waid=from_waid,
                                    body=f"Please send the outfit image (catalog/product photo).",
                                    phone_number_id=outbound_pnid,
                                )
                            return
        ...
        if context_obj and context_obj.get("id"):
            ...
            async for db in get_db():
                try:
                    ...
                    required_gender = (attrs_db.get("type") or attrs_text.get("type"))
                    logging.info(f"f----set required_gender={required_gender}----")
                    required_gender = required_gender.strip().lower() 
